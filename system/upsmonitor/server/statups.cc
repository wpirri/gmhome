...
int main(int argc, char** argv, char** env)
{
  ...
  /* por defecto se conecta al COM 1 */
  ...
  for(i = 1; i < argc; i++)
  {
    ...
    {
      i++;
      strncpy(ups, argv[i], 9);
    }
    else if( !strcmp(argv[i], "-b"))
    {
      i++;
      strncpy(bat, argv[i], 9);
    }
  }

  m_pServer = new CGMServerWait;
  m_pServer->Init("gmh_svupsmon");
  m_pServer->m_pLog->Add(1, "Iniciando server");

  if(wpower_init(port, ups, bat) == 0)
  {
    syslog(LOG_INFO, "MONITOR-UPS: Iniciado");
  }
  else
  {
    m_pServer->m_pLog->Add(1, "UPS desconectada");
		syslog(LOG_ERR, "MONITOR-UPS: UPS desconectada o en estado de alarma al momento de arranque");
  }
  ...
  time_count = 0;
  wait = Check();
  while((rc = m_pServer->Wait(fn, &typ, ch, 1, &inlen, wait * 100)) >= 0)
  {
    if(rc > 0)
    {
...
void OnClose(int sig)
{
  m_pServer->m_pLog->Add(50, "Exit on signal %i", sig);
